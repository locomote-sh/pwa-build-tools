...
async function make( opts, source, target ) {
    ...
    try {
        ...
        if( !pwa ) {
            // Only generate images if manifest has a pwa section.
            return;
        }
        const { background_color } = pwa;
...
async function buildImages( sourcePath, baseImagePath, imageDefs, targetPath, bgColor ) {
    for( const { target, w, h } of imageDefs ) {
        const imagePath = `${target}/${w}x${h}.png`;
        const targetImagePath = Path.join( targetPath, imagePath );
        // Ensure target dir exists.
        await ensureDir( Path.dirname( targetImagePath ) );
        // Check for existing image at target size.
        const existingImagePath = await findFileMatch( sourcePath, imagePath, ['png']);
        if( existingImagePath ) {
            // If pre-scaled copy of image provided then copy to target.
            const srcPath = Path.relative('.', existingImagePath );
            console.error('\tCopying %s -> %s', srcPath, targetImagePath );
            await cp( existingImagePath, targetImagePath );
        }
        else {
            // Else generate scaled copy of image from base image.
            const srcPath = Path.relative('.', baseImagePath );
            console.error('\tScaling %s -> %s', srcPath, targetImagePath );
...
function scaleImage( source, target, width, height, bgColor = 'black' ) {
    return new Promise( ( resolve, reject ) => {
        ...
        sharp( source )
            ...
            .toFile( target, err => err ? reject( err ) : resolve() );
    });
}
