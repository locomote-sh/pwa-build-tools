    ...
    try {
        ...
        */
        ...
        if( serviceWorker ) {
            ...
            const { version = SWCurrentVersion } = serviceWorker;
            const src = SWClientURL( version );
            // The service worker URL. Note that it defaults here to a script under
            // the root path - we are relying on Locomote's relocatable pages feature
            // to rewrite the URL when the site is accessed under a repo path.
            const url = opts.serviceWorkerURL || '/sw.js';
            print();
            print(`<link ref="locomote-service-worker" href="${url}" />`);
