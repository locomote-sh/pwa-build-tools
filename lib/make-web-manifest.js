 ...
 *                  file location is '{target}/manifest.webmanifest'.
...
async function make( opts, source, target, save ) {
    ...
    try {
        ...
        if( !pwa ) {
            // Only generate web manifest if locomote manifest has a pwa section.
            return;
        }
        ...
        const webManifest = Object.assign(
            ...
            pwa,                // Values from Locomote content manifest
            sourceWebManifest   // Values from a user-generated web manifest
        );

        // Save to file if requested.
        if( save ) {
            let path = Path.join( target, 'manifest.webmanifest');
            console.error('Writing %s...', path );
            await writeJSON( path, webManifest );
        }
